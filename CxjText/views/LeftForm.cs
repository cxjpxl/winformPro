...
namespace CxjText.views
{
    public partial class LeftForm : Form, DataClickInface
    {
        ...
        public void setComplete(EnventInfo enventInfo) {
                ...
                if (enventInfo.bangchangType == 0)//默认的情况下
                {
                    if (isBanChang)
                    {
                        ...
                        for (int i = 0; i < searchArray.Count; i++)
                        {
                            ...
                            if (StringComPleteUtils.isPingBanDa07(data,0.7))
                            {
                                obj = searchArray[i];
                                selectDaXiao = false;
                                isBanChang = true;
            ...
            if (autoTempData != null && FormUtils.getCurrentTime() - autoTempData.time < 25000)
            {
                return;
            }
            else
        ...
        //危险下注
        public void weixianXiaZhu(EnventInfo enventInfo)
        {
            ...
            object obj = null; //要下注的对象
            //记得判断时间
            int bangchangInt = DataUtils.renyiTime(searchArray[0], userInfo.tag);
            if (bangchangInt == -1) return;
            /**********************处理自动下注开始*****************************/
            ...
            if (bangchangInt == 0)
            {
                ...
                for (int i = 0; i < searchArray.Count; i++)
                {
                    ...
                    if (StringComPleteUtils.isPingBanDa07(data, 0.5))
                    {
                        obj = searchArray[i];
                        break;
                    }
        ...
        public void setJiaoQiuComplete(EnventInfo enventInfo)
        {
            ...
            object obj = null; //要下注的对象
                               /**********************处理自动下注开始*****************************/
            if (enventInfo.inputType == 0)  //选择让球的情况
            {
                if (isBanChang)
                {
                ...
                //什么都找不到  而且设置默认的情况下
                if (obj == null && enventInfo.bangchangType == 0) {
                    enventInfo.inputType = 1; //转变为下大小
                    isBanChang = true;
                }
            }
