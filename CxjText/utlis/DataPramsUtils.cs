   ...
   public class DataPramsUtils
    {
        ...
        /*******************R系统获取数据***********************************/
        public static String getRData1(UserInfo userInfo)
        {
           ...
           //headJObject["Referer"] = userInfo.dataUrl.Replace("www", "mkt") + "/foot/index";
            ...
            int dataPages = containerNode.GetAttributeValue("data-pages", 1);
            //循环获取分页数据
            for (int page = 2; page <= dataPages; page++)
            {
                getDataUrl = userInfo.dataUrl.Replace("www", "mkt") + "/foot/redata/" + page;
                headJObject = new JObject();
                headJObject["Host"] = userInfo.baseUrl.Replace("www", "mkt");
                headJObject["Origin"] = userInfo.dataUrl.Replace("www", "mkt");
                headJObject["Referer"] = userInfo.dataUrl.Replace("www", "mkt") + "/foot/re";
                rlt = HttpUtils.HttpPostHeader(getDataUrl, "", "", userInfo.cookie, headJObject);
                if (String.IsNullOrEmpty(rlt)) break;//?

                htmlNodes = htmlDoc.DocumentNode.SelectNodes("//tbody");
                if (htmlNodes == null || htmlNodes.Count <= 1)
                {
                    break;
                }
